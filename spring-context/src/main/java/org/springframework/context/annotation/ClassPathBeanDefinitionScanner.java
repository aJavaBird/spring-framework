		...
		for (String basePackage : basePackages) {
			...
			for (BeanDefinition candidate : candidates) {
				...
				if (candidate instanceof AbstractBeanDefinition) {
					/**
					 * 判断是不是 AbstractBeanDefinition 的子类型，
					 * 如果是，则设置默认值 LazyInit、AutowireMode、InitMethodName、DestroyMethodName 等
					 * */
				...
				if (candidate instanceof AnnotatedBeanDefinition) {
					/**
					 * 检查并处理常用注解
					 * 主要是把常用注解的值设置到 BeanDefinition 中
					 * 前面设置了默认值，这里设置真实注解值（包括Lazy、Primary、DependsOn 等注解）
					 * */
